...
public class QueryTestSuite extends JUnitTestCase {
    ...
    public void testQueryParameterPositional(){
        ...
        Query query = em.createNamedQuery("jpa22Employee.findAllEmployeesByFirstNameAndLastNamePos").setParameter(1, "Bob").setParameter(2, "Way");
        int i = 0;
        ...
        query = em.createNamedQuery("jpa22Employee.findAllEmployeesByFirstNameAndLastNamePos", Employee.class).setParameter(1, "Bob");
        try{
    ...
    public void testQueryParameterNamed(){
        EntityManager em = createEntityManager();
        Query query = em.createNamedQuery("jpa22Employee.findAllEmployeesByFirstNameAndLastNameName").setParameter("firstName", "Bob").setParameter("lastName", "Way");
        int i = 0;
        for (Parameter parameter: query.getParameters()){
            assertTrue("Parameter returned wrong position.", parameter.getPosition() == null );
            assertTrue("Parameter returned name.", parameter.getName() != null );
    ...
    public void testConstructorResultQuery() {
        if (getPlatform().isMySQL()) {
            ...
            try {
                ...
                em.createNativeQuery("SELECT o.ORDER_ID AS 'O_ID', o.QUANTITY AS 'O_QUANTITY', i.NAME AS 'O_ITEM' FROM JPA22_ORDER o, JPA22_ITEM i WHERE o.ITEM_ID = i.ID", "OrderConstructorResult").getResultList();
